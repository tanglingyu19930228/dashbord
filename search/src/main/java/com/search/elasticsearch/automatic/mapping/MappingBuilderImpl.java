...
import com.search.elasticsearch.automatic.mapping.annotations.*;
...
import com.search.elasticsearch.automatic.mapping.exceptions.InvalidCustomJsonException;
import com.search.elasticsearch.automatic.mapping.exceptions.InvalidDocumentClassException;
import com.search.elasticsearch.automatic.mapping.exceptions.MappingBuilderException;
import com.search.elasticsearch.automatic.mapping.exceptions.MaxRecursiveLevelClassException;
import com.search.elasticsearch.automatic.mapping.values.DateFieldValue;
import com.search.elasticsearch.automatic.mapping.values.NumericFieldValue;
import com.search.elasticsearch.automatic.mapping.values.RangeFieldValue;
...
public class MappingBuilderImpl implements MappingBuilder {
    ...
    public void validateElasticDocument() {
        if (!isElasticDocument(docClass)) {
            throw new InvalidDocumentClassException("Document Class[" + docClass.getCanonicalName() + "] Invalid. @ElasticDocument must be present.");
        }
    }
    ...
    public static boolean isElasticFieldAnnotation(Annotation annotation) {
        return (
                //Ignore
                annotation instanceof ElasticIgnored
                        //String datatypes
        ...
        if (fields != null && fields.length > 0) {
            ...
            for (Field field : fields) {
                if (containElasticFieldAnnotation(field)) {
                    ...
                    } else if (field.isAnnotationPresent(ElasticIgnored.class)) {
                        //do nothing
                    } else {
    ...
    public XContentBuilder innerBuild(boolean pretty) throws IOException {
        ...
        ElasticDocument elasticDocument = (ElasticDocument) docClass.getAnnotation(ElasticDocument.class);

        //_routing
        if (true || isValueEnabled(elasticDocument.requiredRouting())) {
